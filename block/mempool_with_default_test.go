...
type BlockBusterWithDefaultMempoolTestSuite struct {
	suite.Suite
	ctx sdk.Context
...
func TestBlockBusterWithDefaultMempoolTestSuite(t *testing.T) {
	suite.Run(t, new(BlockBusterWithDefaultMempoolTestSuite))
}

func (suite *BlockBusterWithDefaultMempoolTestSuite) SetupTest() {
	...
	// Base lane set up with default mempool
	...
	suite.baseLane = defaultlane.NewDefaultLaneWithMempool(
		baseConfig,
	...
	var err error
	suite.mempool, err = block.NewLanedMempool(log.NewTestLogger(suite.T()), suite.lanes)
...
func (suite *BlockBusterWithDefaultMempoolTestSuite) TestInsert() {
	cases := []struct {
		name               string
...
func (suite *BlockBusterWithDefaultMempoolTestSuite) TestRemove() {
	cases := []struct {
		name       string
		numTobTxs  uint64
		numBaseTxs uint64
...
func (suite *BlockBusterWithDefaultMempoolTestSuite) fillBaseLane(numTxs uint64) {
...
func (suite *BlockBusterWithDefaultMempoolTestSuite) fillTOBLane(numTxs uint64) {
	for i := uint64(0); i < numTxs; i++ {
...
// fillFreeLane fills the free lane with numTxs transactions that are randomly created.
func (suite *BlockBusterWithDefaultMempoolTestSuite) fillFreeLane(numTxs uint64) {
	for i := uint64(0); i < numTxs; i++ {
		// randomly select an account to create the tx
		randomIndex := suite.random.Intn(len(suite.accounts))
		acc := suite.accounts[randomIndex]
